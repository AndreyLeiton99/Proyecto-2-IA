...
class AlphaBeta:
    ...
    def best_move(self, depth, state, curr_player, alpha, beta):
        ...
        for col in range(7):
            if self.is_legal_move(col, state):
                ...
                legal_moves[col] = -self.search(depth - 1, temp, opp_player, -beta, -alpha)
    ...
    def search(self, depth, state, curr_player, alpha, beta):
        ...
        for child in legal_moves:
            alpha = max(alpha, -self.search(depth - 1, child, opp_player, -beta, -alpha))
            if alpha >= beta:
                break
